//! Citadel WASM - Browser P2P client for the Citadel network
//!
//! Provides WebRTC DataChannels for direct P2P communication and
//! HTTP-based DHT operations for browsers.
...
use web_sys::{
    CloseEvent, ErrorEvent, MessageEvent, RtcConfiguration, RtcDataChannel, RtcDataChannelInit,
...
use tgp_udp::{create_tgp_packet, packet_types, parse_tgp_packet, peer_id_to_hex};

pub mod dht_minimal;
pub use dht_minimal::hash_key;
...
pub struct P2pClient {
    relay_url: String,
    websocket: Rc<Mutex<Option<WebSocket>>>, // Used only for signaling
    peer_id: String,
    peer_hex: u64, // Hex coordinate for TGP routing
    on_block: Rc<Mutex<Option<js_sys::Function>>>,
    on_peer_discovered: Rc<Mutex<Option<js_sys::Function>>>,
    on_dht_response: Rc<Mutex<Option<js_sys::Function>>>, // DHT response callback
    peer_connections: Rc<Mutex<HashMap<String, PeerConnection>>>,
}
...
impl P2pClient {
    ...
    pub fn new(relay_url: String) -> P2pClient {
        ...
        P2pClient {
            ...
            peer_id,
            peer_hex,
            on_block: Rc::new(Mutex::new(None)),
            on_peer_discovered: Rc::new(Mutex::new(None)),
            on_dht_response: Rc::new(Mutex::new(None)),
            peer_connections: Rc::new(Mutex::new(HashMap::new())),
        }
    }

    /// Connect to the relay WebSocket
    pub fn connect(&mut self) -> Result<(), JsValue> {
        console_log!("🔌 Connecting to Citadel relay: {}", self.relay_url);
                    ...
                    if let Some(msg_type) = json.get("type").and_then(|v| v.as_str()) {
                        match msg_type {
                            "browser_peers" => {
                                ...
                                if let Some(peers_arr) =
                                    json.get("peers").and_then(|v| v.as_array())
                                {
                                    console_log!(
                                        "👥 Received {} browser peer(s) to connect to",
                                        peers_arr.len()
                                    );
                                    ...
                                    if let Some(ref callback) = *on_peer_discovered.lock().unwrap()
                                    {
                                        for peer_info in peers_arr {
                                            if let Some(peer_id_str) =
                                                peer_info.get("peer_id").and_then(|v| v.as_str())
                                            {
                                                if peer_id_str != my_peer_id {
                                                    console_log!(
                                                        "🔗 Discovered browser peer: {}",
                                                        peer_id_str
                                                    );
                                                    let peer_id_js = JsValue::from_str(peer_id_str);
                                                    let _ =
                                                        callback.call1(&JsValue::NULL, &peer_id_js);
                                                }
                                            }
                                        }
                                    }
                            ...
                            "browser_joined" => {
                                ...
                                if let Some(peer_id_str) =
                                    json.get("peer_id").and_then(|v| v.as_str())
                                {
                                    if peer_id_str != my_peer_id {
                                        ...
                                        if let Some(ref callback) =
                                            *on_peer_discovered.lock().unwrap()
                                        {
                                            let peer_id_js = JsValue::from_str(peer_id_str);
    ...
    pub fn create_delete_featured_release_block(
        &self,
    ...
    pub fn disconnect(&self) -> Result<(), JsValue> {
        ...
        if let Some(ref ws) = *ws {
            ws.close()?;
            console_log!("🔌 Disconnected from Citadel relay");
        }
        Ok(())
    }
    ...
    pub fn broadcast_block_direct(&self, block_json: JsValue) -> Result<(), JsValue> {
        let block = UBTSBlock::from_js(block_json)?;
        let block_bytes =
            serde_json::to_vec(&block).map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        console_log!(
            "📡 TGP packets sent to {} peer(s) via direct P2P",
            sent_count
        );
    ...
    pub async fn dht_get(&self, key_hex: String) -> Result<(), JsValue> {
        ...
        let request_payload =
            serde_json::to_vec(&request).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub async fn dht_put(&self, key_hex: String, value: Vec<u8>) -> Result<(), JsValue> {
        ...
        if let Some(ref ws) = *ws {
            ws.send_with_u8_array(&packet)?;
            console_log!(
                "📤 Sent DHT PUT request for key={} value_len={}",
                key_hex,
                value.len()
            );
        } else {
    ...
    pub fn announce_discovered_peers(&self) -> Result<(), JsValue> {
        ...
        if let Some(ref ws) = *ws {
            ...
            console_log!(
                "🌉 Announced {} discovered peers to Citadel relay (helping nodes find each other!)",
                connected_peers.len()
            );
        } else {
            return Err(JsValue::from_str("WebSocket not connected"));
